    ...
    <main class="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
                ...
                @if($nextEvent)
                    <p class="mt-1 text-sm text-stone-600">{{ $nextEvent->all_day ? $nextEvent->starts_at->format('d.m.Y').' · Ganztägig' : $nextEvent->starts_at->format('d.m.Y H:i') }} · {{ $nextEvent->ownerDisplayName() }}</p>
                @endif
            </div>
        </div>
                    ...
                    <div class="py-3">
                        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                ...
                                <p class="text-sm text-stone-600">{{ $event->all_day ? $event->starts_at->format('d.m.Y').' · Ganztägig' : $event->starts_at->format('d.m.Y H:i') }} · {{ $event->ownerDisplayName() }}</p>
                            </div>
