                            ...
                            <div class="min-h-36 border-r border-stone-200 p-3 last:border-r-0 {{ $day['is_current_month'] ? 'bg-white' : 'bg-stone-50 text-stone-400' }}">
                                ...
                                <div class="space-y-1.5">
                                    @foreach($day['events'] as $event)
                                        <button class="flex w-full items-center gap-2 rounded-md px-2 py-1 text-left text-xs hover:bg-stone-100" type="button" data-calendar-event='@json([
                                            'title' => $event->title,
                    ...
                    <section class="p-4 {{ $day['is_current_month'] ? 'bg-white' : 'bg-stone-50 text-stone-400' }}">
                        ...
                        <div class="mt-3 space-y-2">
                            @forelse($day['events'] as $event)
                                <button class="flex w-full items-center gap-2 rounded-md bg-stone-50 p-3 text-left text-sm hover:bg-stone-100" type="button" data-calendar-event='@json([
                                    'title' => $event->title,
                                    'time' => $event->dashboardTimeLabel(),
                                    'date' => $event->starts_at->format('d.m.Y'),
                                    'person' => $event->ownerDisplayName(),
